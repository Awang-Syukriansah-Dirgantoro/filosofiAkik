...
export default function AkikCard({
  ...
  return (
    <div
      className="bg-white rounded-xl shadow p-2 flex flex-col gap-1 w-full min-h-[320px] max-h-[400px] cursor-pointer hover:shadow-lg transition"
      onClick={() => id && router.visit(`/product/${id}`)}
    >
      <div className="relative h-[60%]">
        <img src={`storage/${image}`} alt={title} className="bg-black rounded-lg w-full h-full object-cover shadow" />
      ...
      <div className="flex-1 flex flex-col gap-1 justify-between my-2">
        <div className='flex flex-col gap-1'>
          <div className="font-bold text-sm md:text-xl text-black">{title}</div>
        ...
        <div>
          {negotiable && <div className="text-xs text-black font-bold">Negotiable</div>}
